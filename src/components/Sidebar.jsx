...
export default function Sidebar() {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      ...
      try {
        const response = await fetch('https://openrouter.ai/api/frontend/v1/catalog/models');
        const data = await response.json();
        // Limit to 20 models for performance in this UI
        const models = data.data.slice(0, 20);
  ...
  useEffect(() => {
    const searchModels = async () => {
      if (modelSearchQuery.length >= 3) {
        setIsLoadingModels(true);
        try {
          const response = await fetch(`https://openrouter.ai/api/frontend/v1/models/find?q=${encodeURIComponent(modelSearchQuery)}`);
          const data = await response.json();
  ...
  return (
    <div
      className={`h-full flex flex-col transition-all duration-200 border-r border-[#e9edef] bg-[#ffffff] ${(activeSessionId && activeMobileTab !== 'profile') ? 'hidden md:flex md:w-[380px] lg:w-[420px]' : 'w-full md:w-[380px] lg:w-[420px]'
        }`}
    >
                ...
                return (
                  <div
                    ...
                    onClick={() => startNewChat({
                      id: model.slug,
                      name: model.name,
                      avatar: `https://robohash.org/${encodeURIComponent(model.name)}?set=set1`,
                  ...
                  >
                    <div className="relative shrink-0 w-8 h-8 flex items-center justify-center rounded-md bg-[#f0f2f5] overflow-hidden">
                      {model.endpoint?.provider_info?.icon?.url ? (
                        <img
                          src={model.endpoint.provider_info.icon.url.startsWith('/') ? `https://openrouter.ai${model.endpoint.provider_info.icon.url}` : model.endpoint.provider_info.icon.url}
                          alt={model.name}
